...
status_codes perform_operation(long long* result, Operation_status operation, long long value1, long long value2)
{
    ...
    {
        case ADDITION:
            if ((value2 > 0 && value1 > LLONG_MAX - value2) ||
                (value2 < 0 && value1 < LLONG_MIN - value2))
            {
                return INVALID_ARGUMENT;
            }
        ...
        case MULTIPLICATION:
            if (value1 > 0)
            {
                ...
                if (value2 < 0 && value2 < LLONG_MIN / value1)
                {
                    return INVALID_ARGUMENT;
                }
            }
            ...
            {
                if (value2 > 0 && value1 < LLONG_MIN / value2)
                {
                    return INVALID_ARGUMENT;
                }
                if (value2 < 0 && (value1 != 0 && value2 < LLONG_MAX / value1))
                {
                    return INVALID_ARGUMENT;
                }
            }
...
status_codes parse_long(const char* str, long long* result)
{
    ...
    *result = strtoll(str, &endptr, 10);
    if (strlen(str) > 19 && *result != 0) return INVALID_ARGUMENT;
    if (*endptr != '\0')
    {
